...
//!! ad_comm::change_datafile_name("modjcq_opt.ctl");
 init_number sigmaR
 init_vector dt(1,2)
...
//-------------------------------------------------------------- 
 ...
 init_vector log_qcru(1,1,opt_qc)
...
//======================================================================
 ...
 for (int i=1;i<ntime;i++){
 	...
 	Nv(i+1,nedades)=Nv(i+1,nedades)+Nv(i,nedades)*exp(-1.0*M);
 }
...
//======================================================================
	...
	NVcru = elem_prod(elem_prod(N,mfexp(-dt(2)*(Z))),Sel_c)*Prob_talla;
	BMflo = NVflo*Wmed;
	BMcru = NVcru*Wmed;
...
//======================================================================
   ...
   for (int i=1;i<=ntime;i++){
                 pred_Bcru(i) = mfexp(log_qcru(1))*BMcru(i);
   }
...
//======================================================================
 ...
 suma3=0; suma4=0; suma5=0; penalty=0;

 likeval(1) = 0.5*suma1;
 likeval(2) = 0.5*suma2;
 likeval(3) = 0.5*norm2(elem_div(log(elem_div(Desemb,pred_Desemb)),cv_index(1)));
 ...
 likeval(5)=1./(2*square(sigmaR))*norm2(dev_log_Ro);}

 if(active(dev_log_No)){
 likeval(6)=1./(2*square(sigmaR))*norm2(dev_log_No);}
